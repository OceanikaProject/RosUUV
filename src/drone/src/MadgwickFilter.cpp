...
#define betaDef         1.2f            // 2 * proportional gain
...
void MadgwickFilter::updateIMU(float gx, float gy, float gz, float ax, float ay, float az) {
  ...
  float q[] = {q0, q1, q2, q3};
//  float result[4];
  float* result = q_rotate(q, a);
  result[3] = result[3] - 1;
  result = q_rotate(q, result);
...
float *MadgwickFilter::q_mult(float q[], float r[])
{
  float result[4];
...
float *MadgwickFilter::q_rotate(float point[], float q[])
{
  float q_conj[] = {q[0], -1*q[1], -1*q[2], -1*q[3]};
  float* result = q_mult(q_mult(q, point), q_conj);
  return result;
}

void q_inverse(float q[])
{
